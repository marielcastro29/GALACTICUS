    ...
    switch($_GET["op"]){
        ...
        case "guardaryeditar":
            if(empty($_POST["suc_id"])){
                $sucursal->insert_sucursal($_POST["suc_nom"]);
            }else{
                $sucursal->update_sucursal($_POST["suc_id"],$_POST["suc_nom"]);
            }
            break;
        ...
        case "listar":
            ...
            foreach($datos as $row){
                ...
                $sub_array[] = '<button type="button" onClick="editar('.$row["SUC_ID"].')" id="'.$row["SUC_ID"].'" class="btn btn-primary btn-icon waves-effect waves-light"><i class="ri-edit-2-line"></i></button>';
